@extends('layouts.app')
@section('breadcrumb', 'Sales / Orders / Edit')
...
<div class="card">
    ...
    <form id="order-form" method="POST" action="{{ route('orders.update', $order->order_id) }}">
        @csrf
        @method('PUT')
                ...
                <select class="input" id="currency-select">
                    @foreach($currencies as $cur)
                        <option value="{{ $cur->id }}"
                                ...
                                {{ $cur->code === $order->currency_code ? 'selected' : '' }}>
                            {{ $cur->code }} &mdash; {{ $cur->name }}
                        </option>
                    @endforeach
                </select>
                <input type="hidden" name="currency_code" id="currency-code-hidden" value="{{ $order->currency_code ?? 'PHP' }}">
            </div>
        </div>
        ...
        <div class="form-grid">
            <div>
                ...
                <input class="input" name="firstname" id="cust_firstname" value="{{ old('firstname', $order->firstname) }}" maxlength="32">
            </div>
            <div>
                <label>Last Name</label>
                <input class="input" name="lastname" id="cust_lastname" value="{{ old('lastname', $order->lastname) }}" maxlength="32">
            </div>
            <div>
                <label>Email</label>
                <input class="input" name="email" value="{{ old('email', $order->email) }}" maxlength="96">
            </div>
            <div>
                <label>Telephone</label>
                <input class="input" name="telephone" value="{{ old('telephone', $order->telephone) }}" maxlength="32">
            </div>
        </div>
        ...
        <div class="form-grid" id="shipping-fields">
            <div>
                ...
                <input class="input ship-field" name="shipping_firstname" data-field="firstname" value="{{ old('shipping_firstname', $order->shipping_firstname) }}" maxlength="32">
            </div>
            <div>
                <label>Last Name</label>
                <input class="input ship-field" name="shipping_lastname" data-field="lastname" value="{{ old('shipping_lastname', $order->shipping_lastname) }}" maxlength="32">
            </div>
            <div>
                <label>Company</label>
                <input class="input ship-field" name="shipping_company" data-field="company" value="{{ old('shipping_company', $order->shipping_company) }}" maxlength="40">
            </div>
            <div>
                <label>Address 1</label>
                <input class="input ship-field" name="shipping_address_1" data-field="address_1" value="{{ old('shipping_address_1', $order->shipping_address_1) }}" maxlength="128">
            </div>
            <div>
                <label>Address 2</label>
                <input class="input ship-field" name="shipping_address_2" data-field="address_2" value="{{ old('shipping_address_2', $order->shipping_address_2) }}" maxlength="128">
            </div>
            <div>
                <label>City</label>
                <input class="input ship-field" name="shipping_city" data-field="city" value="{{ old('shipping_city', $order->shipping_city) }}" maxlength="128">
            </div>
            <div>
                <label>Postcode</label>
                <input class="input ship-field" name="shipping_postcode" data-field="postcode" value="{{ old('shipping_postcode', $order->shipping_postcode) }}" maxlength="10">
            </div>
            <div>
                <label>Country</label>
                <input class="input ship-field" name="shipping_country" data-field="country" value="{{ old('shipping_country', $order->shipping_country) }}" maxlength="128">
            </div>
            <div>
                <label>Zone / Region</label>
                <input class="input ship-field" name="shipping_zone" data-field="zone" value="{{ old('shipping_zone', $order->shipping_zone) }}" maxlength="128">
            </div>
            <div>
                <label>Shipping Method</label>
                <input class="input" name="shipping_method" value="{{ old('shipping_method', $order->shipping_method) }}">
            </div>
        </div>
        ...
        <div class="d-flex items-center gap-8" style="margin-top:24px;">
            ...
            <label class="d-flex items-center gap-6" style="font-size:13px; font-weight:500; cursor:pointer;">
                <input type="checkbox" id="payment-same-as-shipping">
        ...
        <div id="payment-fields">
            <div class="form-grid" style="margin-top:12px;">
                <div>
                    <label>First Name</label>
                    <input class="input pay-field" name="payment_firstname" data-field="firstname" value="{{ old('payment_firstname', $order->payment_firstname) }}" maxlength="32">
                </div>
                <div>
                    <label>Last Name</label>
                    <input class="input pay-field" name="payment_lastname" data-field="lastname" value="{{ old('payment_lastname', $order->payment_lastname) }}" maxlength="32">
                </div>
                <div>
                    <label>Company</label>
                    <input class="input pay-field" name="payment_company" value="{{ old('payment_company', $order->payment_company) }}" maxlength="40">
                </div>
                <div>
                    <label>Address 1</label>
                    <input class="input pay-field" name="payment_address_1" data-field="address_1" value="{{ old('payment_address_1', $order->payment_address_1) }}" maxlength="128">
                </div>
                <div>
                    <label>Address 2</label>
                    <input class="input pay-field" name="payment_address_2" data-field="address_2" value="{{ old('payment_address_2', $order->payment_address_2) }}" maxlength="128">
                </div>
                <div>
                    <label>City</label>
                    <input class="input pay-field" name="payment_city" data-field="city" value="{{ old('payment_city', $order->payment_city) }}" maxlength="128">
                </div>
                <div>
                    <label>Postcode</label>
                    <input class="input pay-field" name="payment_postcode" data-field="postcode" value="{{ old('payment_postcode', $order->payment_postcode) }}" maxlength="10">
                </div>
                <div>
                    <label>Country</label>
                    <input class="input pay-field" name="payment_country" data-field="country" value="{{ old('payment_country', $order->payment_country) }}" maxlength="128">
                </div>
                <div>
                    <label>Zone / Region</label>
                    <input class="input pay-field" name="payment_zone" data-field="zone" value="{{ old('payment_zone', $order->payment_zone) }}" maxlength="128">
                </div>
                <div>
                    <label>Payment Method</label>
                    <input class="input" name="payment_method" value="{{ old('payment_method', $order->payment_method) }}" maxlength="128">
                </div>
            </div>
        </div>
        ...
        <input type="hidden" name="total" id="order-total-hidden" value="{{ $order->total }}">
            ...
            <div>
                ...
                <select class="input" name="order_status_id" id="order-status-select">
                    @foreach($statuses as $s)
                        <option value="{{ $s->order_status_id }}" {{ (int)old('order_status_id', $order->order_status_id) === $s->order_status_id ? 'selected' : '' }}>{{ $s->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label>Tracking Number</label>
                <input class="input" name="tracking_number" value="{{ old('tracking_number', $order->tracking_number) }}" maxlength="32">
            </div>
            <div class="full">
                <label>Order Comment</label>
                <textarea class="input" name="comment" rows="2">{{ old('comment', $order->comment) }}</textarea>
            </div>
        </div>
    </form>
    ...
    <table class="table">
        ...
        @foreach($history as $h)
            <tr>
                <td class="text-nowrap">{{ $h->date_added }}</td>
                <td>{{ $h->user_name ?? '-' }}</td>
                <td>{{ $h->status->name ?? '-' }}</td>
                <td>{{ $h->comment }}</td>
            </tr>
    ...
    <table class="table">
        ...
        @foreach($orderTotals as $ot)
            <tr>
                <td>{{ $ot->title }}</td>
                <td class="text-right" style="width:140px;">{{ number_format($ot->value, 2) }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
    ...
    {{-- Save / Delete --}}
    <div class="d-flex" style="justify-content:space-between; align-items:center; margin-top:24px;">
        <div>
            <form id="delete-order-form" method="POST" action="{{ route('orders.destroy', $order->order_id) }}" class="hidden">
                @csrf
                @method('DELETE')
            </form>
            <button class="btn danger" type="button"
                data-confirm="Delete this order and its history?" data-confirm-submit="delete-order-form">
                Delete Order
            </button>
        </div>
        <button class="btn" type="submit" form="order-form">Save Order</button>
    </div>
</div>
...
(function() {
    ...
    var lineItems = []; // each: { product_id, name, model, sku, basePrice (in order currency), baseCost, qty, option_value_id, option_name, option_value }

    // Pre-populate from existing order products
    ...
    lineItems.push({
        ...
        option_name: @json($p->options->first()->name ?? ''),
        option_value: @json($p->options->first()->value ?? ''),
        priceIsDisplay: true
    });
    @endforeach
    ...
    function recalcTotal() {
        ...
        lineItems.forEach(function(item) {
            // For pre-populated items, price is already in order currency
            // For newly added items via modal, basePrice is in PHP and needs conversion
            var price = item.priceIsDisplay ? item.basePrice : convertPrice(item.basePrice);
    ...
    function renderProducts() {
        ...
        lineItems.forEach(function(item, idx) {
            ...
            var displayPrice = item.priceIsDisplay ? item.basePrice : convertPrice(item.basePrice);
            var displayCost = item.priceIsDisplay ? item.baseCost : convertPrice(item.baseCost);
            ...
            var hiddens = [
                ...
                { n: 'cost', v: displayCost.toFixed(2) },
    ...
    document.addEventListener('input', function(e) {
        ...
        if (e.target.classList.contains('line-price')) {
            ...
            // Mark as display price since user edited in display currency
            lineItems[idx].basePrice = newDisplayPrice;
            lineItems[idx].priceIsDisplay = true;
            // Update hidden input
        ...
        if (e.target.classList.contains('line-qty')) {
            ...
            cur = getSelectedCurrency();
            var displayPrice = lineItems[idx].priceIsDisplay ? lineItems[idx].basePrice : convertPrice(lineItems[idx].basePrice);
            var row = e.target.closest('tr');
