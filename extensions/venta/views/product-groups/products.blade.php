@extends('layouts.app')
@section('title', $group->name . ' — ' . $setting->store_name . ' Products')
@section('breadcrumb', 'Marketplace / ' . $setting->store_name . ' / ' . $group->name . ' / Products')

@section('content')
    <div class="page-header">
        <div>
            <h2>{{ $group->name }} — Products</h2>
            <div class="text-muted text-sm">{{ $setting->store_name }} — push catalog products, sync stock &amp; price.</div>
        </div>
        <div class="page-header-actions">
            <a class="btn secondary" href="{{ route('ext.venta.product-groups.edit', [$setting->id, $group->id]) }}">Edit Group</a>
    ...
    <div class="card mb-12">
        <form method="GET" action="{{ route('ext.venta.product-groups.products', [$setting->id, $group->id]) }}" style="display:grid; grid-template-columns:1fr auto auto auto; gap:10px; align-items:end;">
            <div>
            ...
            <div class="d-flex gap-6">
                ...
                @if(($q ?? '') !== '' || ($syncStatus ?? 'all') !== 'all' || ($erpStatus ?? 'all') !== 'all')
                    <a href="{{ route('ext.venta.product-groups.products', [$setting->id, $group->id]) }}" class="btn small secondary">Clear</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="card">
        ...
        <div id="bulk-bar" style="display:none; padding:10px 16px; margin-bottom:12px; background:var(--accent-light); border:1px solid rgba(59,130,246,.2); border-radius:var(--radius-md); align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;">
            ...
            <div class="d-flex gap-6 flex-wrap">
                <form method="POST" action="{{ route('ext.venta.product-groups.push', [$setting->id, $group->id]) }}" class="bulk-action-form bulk-push-form" style="margin:0;">
                    @csrf
                    <button type="submit" class="btn small">Push to Venta</button>
                </form>
                <form method="POST" action="{{ route('ext.venta.product-groups.push-prices', [$setting->id, $group->id]) }}" class="bulk-action-form" style="margin:0;">
                    @csrf
                    <button type="submit" class="btn small">Sync Price</button>
                </form>
                <form method="POST" action="{{ route('ext.venta.product-groups.push-stock', [$setting->id, $group->id]) }}" class="bulk-action-form" style="margin:0;">
                    @csrf
                    <button type="submit" class="btn small">Sync Qty</button>
                </form>
                <form method="POST" action="{{ route('ext.venta.product-groups.sync-ids', [$setting->id, $group->id]) }}" class="bulk-action-form" style="margin:0;">
                    @csrf
                    <button type="submit" class="btn small">Sync Venta ID</button>
                </form>
                <form method="POST" action="{{ route('ext.venta.product-groups.mass-remove', [$setting->id, $group->id]) }}" class="bulk-action-form" style="margin:0;">
                    @csrf
        ...
        <div class="table-wrap">
            <table class="table" id="ventaProductsTable">
                <thead>
                <tr>
                    ...
                    <th style="width:130px;">Venta ID</th>
                ...
                @forelse($products as $p)
                    @php
                        ...
                        $hasVentaId = !empty($p->venta_product_id) && !$isUnlinked;
                        $hasError = in_array($syncState, ['error', 'failed']) && ($p->push_error ?? null);
                    ...
                    <tr>
                                ...
                                <div style="margin-top:4px;">
                                    @foreach($optRows as $or)
                                        ...
                                        <div class="text-xs" style="color:var(--text-secondary); line-height:1.6;">
                                            {{ $ovn ?: '-' }}@if($osku) <span class="text-muted">&mdash; {{ $osku }}</span>@endif
                        ...
                        <td>
                            @if($hasVentaId)
                                <span style="font-size:12px; font-family:monospace;">{{ $p->venta_product_id }}</span>
                            @else
                        ...
                        <td>
                            ...
                            @elseif(in_array($syncState, ['error', 'failed']))
                        ...
                        <td>
                            @if($hasError)
                                <div class="text-xs" style="color:var(--danger); word-break:break-word; line-height:1.4;">
                                    {{ \Illuminate\Support\Str::limit($p->push_error, 80) }}
                                </div>
                                @if($p->last_pushed_at)
                                    <div class="text-xs text-muted" style="margin-top:2px;">{{ \Carbon\Carbon::parse($p->last_pushed_at)->diffForHumans() }}</div>
                                @endif
                            @elseif($hasVentaId && $p->last_pushed_at)
                        ...
                        <td>
                            <div style="position:relative;" class="row-actions">
                                ...
                                <div class="action-menu" style="display:none; position:absolute; right:0; top:100%; margin-top:4px; z-index:50; background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); box-shadow:var(--shadow-lg); min-width:170px; padding:4px 0;">
                                    ...
                                    @if(!$hasVentaId && !$isUnlinked)
                                        <form method="POST" action="{{ route('ext.venta.product-groups.sync-id', [$setting->id, $group->id, $p->product_id]) }}" style="margin:0;" class="single-action-form">
                                            @csrf
                                        ...
                                        <form method="POST" action="{{ route('ext.venta.product-groups.push', [$setting->id, $group->id]) }}" style="margin:0;" class="single-action-form" data-pid="{{ $p->product_id }}">
                                            @csrf
                                    ...
                                    @endif
                                    @if($hasVentaId)
                                        <form method="POST" action="{{ route('ext.venta.product-groups.push', [$setting->id, $group->id]) }}" style="margin:0;" class="single-action-form">
                                            @csrf
                                            <input type="hidden" name="ids[]" value="{{ $p->product_id }}">
                                            <button type="submit" class="action-item">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/></svg>
                                        ...
                                        <form method="POST" action="{{ route('ext.venta.product-groups.push-stock', [$setting->id, $group->id]) }}" style="margin:0;" class="single-action-form">
                                            @csrf
                                            <input type="hidden" name="ids[]" value="{{ $p->product_id }}">
                                            <button type="submit" class="action-item">
                                        ...
                                        <form method="POST" action="{{ route('ext.venta.product-groups.push-prices', [$setting->id, $group->id]) }}" style="margin:0;" class="single-action-form">
                                            @csrf
                                            <input type="hidden" name="ids[]" value="{{ $p->product_id }}">
                                            <button type="submit" class="action-item">
                                        ...
                                        <form method="POST" action="{{ route('ext.venta.product-groups.unlink', [$setting->id, $group->id, $p->product_id]) }}" style="margin:0;" class="single-action-form" data-confirm="Unlink this product from Venta? The product stays in the group but sync will stop.">
                                            @csrf
                                        ...
                                        <form method="POST" action="{{ route('ext.venta.product-groups.deleteFromVenta', [$setting->id, $group->id, $p->product_id]) }}" style="margin:0;" class="single-action-form" data-confirm="Delete this product from the Venta store? This cannot be undone.">
                                            @csrf
                                    ...
                                    @endif
                                    @if($isUnlinked)
                                        <form method="POST" action="{{ route('ext.venta.product-groups.link', [$setting->id, $group->id, $p->product_id]) }}" style="margin:0;" class="single-action-form">
                                            @csrf
                                            <button type="submit" class="action-item">
                                    ...
                                    @if(in_array((int)$p->product_id, $manualIds))
                                        <div style="border-top:1px solid var(--border); margin:4px 0;"></div>
                                        <form method="POST" action="{{ route('ext.venta.product-groups.removeProduct', [$setting->id, $group->id, $p->product_id]) }}" style="margin:0;" data-confirm="Remove this product from the group?">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="action-item" style="color:var(--danger);">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
                                                Remove from Group
                ...
                @empty
                    <tr>
                        <td colspan="13" class="text-muted">No products found. Edit the group to add products.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
    ...
    <div class="card mt-16">
        ...
        <form id="add-products-form" method="POST" action="{{ route('ext.venta.product-groups.addProducts', [$setting->id, $group->id]) }}">
            @csrf
        </form>
...
<script>
(function(){
    var table = document.getElementById('ventaProductsTable');
    if (!table) return;
    ...
    document.querySelectorAll('form.bulk-action-form').forEach(function(form){
        form.addEventListener('submit', function(e){
            ...
            if (form.classList.contains('bulk-push-form')) {
                confirmModal('Push ' + ids.length + ' selected product(s) to Venta? Already-synced products will be skipped.').then(function(ok) {
